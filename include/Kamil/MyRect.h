 ...
 * sf::FloatRect  is a templated class of sf::Rect<float> and its primary use is
 * for defining the border and creating a hollow rectangle, as such it only has
 * methods for collision detection and intersections. The normal RectangleShape
 * class creates a basic rectangle without the collision and intersections
 * checking so we inherit this functionality from FloatRect and in effect add it
 * to the instantiated RectangleShape in the MyRect class.
 *
 * The sf::Drawable is only here to add a draw property to our class so when we
 * draw to the RenderTarget, in this case RenderWindow, we can use the same code
 * of window.draw(our_own_object) instead of the general
 * our_own_object.draw(window). This is done so when others use this code it
 * makes it easier for them to follow a standard way of drawing to the
 * RenderTarget and not having to worry about passing parameters into the
 * objects.
 *
 */
...
public:
   ...
   */
  MyRect(sf::Vector2f pos, sf::Vector2f size, sf::Color fillColour,
         sf::Color outlineColour, float outlineThicknes);
  MyRect();
